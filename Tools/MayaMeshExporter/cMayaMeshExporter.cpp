        ...
        // Fill in the material info
        {
            ...
            for ( size_t i = 0; i < shadingGroupCount; ++i )
            {
                ...
                if ( status )
                {
                    ...
                    {
                        const auto getConnectionsWithThisAsDestination = true;
                        const auto dontGetConnectionsWithThisAsSource = false;
...
//          fout << std::fixed;
            ...
            // Write vertex data
            {
                fout << "\t-- This table contains a list of vertices and their corresponding colors\n"
                    "\tvertexData = \n"
                    "\t{\n";

                size_t count = i_vertexArray.size();
                for (const auto& vertex : i_vertexArray)
                {
                    --count;
                    fout << "\t\t{\n";
            ...
            // Write index data
            {
                fout << "\n\t-- This table contains a list of indices that map to the vertices above\n"
                    "\tindexData = \n"
                    "\t{\n\t\t";

                size_t count = i_indexArray.size();
                for (const auto& index : i_indexArray)
                {
                    --count;
                    fout << index << (count == 0 ? "\n" : ", ");
                }
