...
void eae6320::Graphics::cSprite::GetVertexData(eae6320::Graphics::VertexFormats::sSprite* o_vertexData) const
{
	...
	// top-right
	o_vertexData[4].x = m_origin.x + m_extents.x;
	o_vertexData[4].y = m_origin.y + m_extents.y;

	// top-left
	o_vertexData[5].x = m_origin.x - m_extents.x;
	o_vertexData[5].y = m_origin.y + m_extents.y;
}
